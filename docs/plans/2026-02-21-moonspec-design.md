...
- `moonrockz/cucumber-expressions` — Cucumber Expression matching (v0.1.0)
- `moonbitlang/async` — Async runtime for parallel scenario execution
...
`moonspec gen` produces `_test.mbt` files from `.feature` files. Each scenario becomes an `async test` block that `moon test` discovers natively.

```moonbit
// Generated by moonspec — do not edit
// Source: features/login.feature

async test "Feature: User Login / Scenario: Valid credentials" {
...
Can be wired as a pre-build hook in `moon.pkg`:
```
...
Call directly inside an `async test` block for maximum simplicity:

```moonbit
async test "BDD: login feature" {
...
pub(open) trait Hooks {
  before_scenario(Self, ScenarioInfo) -> async Unit!Error
  after_scenario(Self, ScenarioInfo, Result) -> async Unit!Error
  before_step(Self, StepInfo) -> async Unit!Error
  after_step(Self, StepInfo, Result) -> async Unit!Error
}
```

All hooks have default no-op implementations. Hooks are `async` to support
async setup/teardown (e.g., database connections, HTTP fixtures).
...
pub struct RunConfig {
  tags : String?
  parallel : Int           // 0 = sequential, >0 = max concurrent scenarios
  dry_run : Bool
  formatters : Array[Formatter]
  timeout : Int?           // per-scenario timeout in milliseconds
}
```

### Async Execution Model

moonspec uses `moonbitlang/async` for concurrent scenario execution. The model is
**single-threaded cooperative** — no data races, no locks, clean World isolation.

Key patterns:
- **`async test` blocks** — generated tests use native `async test` for seamless
  integration with `moon test`
- **`@async.all` with `max_concurrent`** — run scenarios in parallel with bounded
  concurrency
- **`allow_failure=true`** — each scenario runs independently; failures don't cancel
  siblings
- **`@async.with_timeout`** — per-scenario timeout support
- **`@async.pause()`** — yield points in CPU-bound step handlers

```moonbit
// Parallel scenario execution with bounded concurrency
async fn run_scenarios_parallel(
  scenarios : Array[async () -> ScenarioResult],
  max_concurrent : Int,
) -> Array[ScenarioResult] {
  @async.all(scenarios[:], max_concurrent~)
}
```
...
When `parallel > 0`, scenarios are wrapped in `async () -> ScenarioResult`
closures and dispatched via `@async.all` with `allow_failure=true`. Each scenario
gets its own fresh World instance, ensuring isolation.
